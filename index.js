...
switch (platform) {
  ...
  case 'win32':
    ...
    break
  case 'darwin':
    localFileExisted = existsSync(join(__dirname, 'keytar-rs.darwin-universal.node'))
    try {
      if (localFileExisted) {
        nativeBinding = require('./keytar-rs.darwin-universal.node')
      } else {
        nativeBinding = require('@traeok/keytar-rs-darwin-universal')
      }
      break
    } catch {}
    switch (arch) {
